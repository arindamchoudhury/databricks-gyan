...
- [catalog-commits write mechanics (SunnyData)](../sunnydata/catalog-commits.md) — practitioner walkthrough (SunnyData/Hubert Dudek) of the **write mechanics** this docs page leaves implicit: the staged-commits 4-step sequence, the `_delta_log/_staged_commits/` folder on storage, the REST inspection endpoint, the DynamoDB→UC framing, and ABAC enforcement on external reads.
