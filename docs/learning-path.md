...
11. **Reference — [Delta Lake table streaming reads and writes](sources/databricks-docs/structured-streaming-delta-lake/)** (captured notes) — Delta as both streaming source and sink. **Retention window gotcha:** stream must run at least once within 7 days (VACUUM files) / 30 days (transaction log); miss the window → `DELTA_FILE_NOT_FOUND_DETAILED`; never set `ignoreMissingFiles=true` (wrong results). Sink: Delta transaction log = exactly-once even with concurrent streams/batches; empty commits at `epochId=-1` are expected (first batch per run, schema change); store checkpoint in `<table-name>/_checkpoints` (VACUUM skips `_` dirs). Backlog metrics: `numBytesOutstanding`, `numFilesOutstanding`, `backlogEndOffset`. Source changes: SS accepts only appends — UPDATE/DELETE/MERGE/OVERWRITE fails the stream; four approaches: `skipChangeCommits` (recommended — ignores changed files, appends only; can toggle on/off), full refresh, CDF, materialized views. Legacy: `ignoreChanges` (DBR ≤11.3, re-emits rewritten rows → duplicates) replaced by `skipChangeCommits` in DBR 12.2+. `foreachBatch` idempotency: `txnAppId`+`txnVersion`; if checkpoint deleted → **must change `txnAppId`** (new checkpoints start at batchId=0; Delta skips `(appId, batchId)` already seen). `startingVersion`/`startingTimestamp`: stream from a past point, but schema always = latest Delta table schema. `withEventTimeOrder` (DBR 11.3+): prevents data drops during initial snapshot when using watermarks — default file-mod-time order can prematurely advance watermark; divides snapshot into event-time buckets; can't change once active; can't downgrade DBR until initial snapshot completes. Rate limiting: `maxFilesPerTrigger` (default 1000), `maxBytesPerTrigger` (soft max). I2 still ⬜.
12. **Reference — [Subscribe to Google Pub/Sub](sources/databricks-docs/streaming-pub-sub/)** (captured notes) — built-in `format("pubsub")` connector; exactly-once from connector side, but Pub/Sub itself may deliver duplicates or out-of-order rows (must handle in code). Auth: `clientId`/`clientEmail`/`privateKey`/`privateKeyId` (use secrets). Required GCP roles: `pubsub.viewer` (check sub exists) + `pubsub.subscriber` (fetch); optional `pubsub.editor` (create sub / `deleteSubscriptionOnStreamStop`). Role gotcha: resource-level grants must apply to both topic AND subscription. Schema: `messageId` (String), `payload` (Array[Byte]), `attributes` (String), `publishTimestampInMillis` (Long). `Trigger.AvailableNow` supported. Metrics include `numDuplicatesSinceStreamStart`. Limitation: no speculative execution. I2 still ⬜.
13. **Reference — [foreachBatch — write to arbitrary data sinks](sources/databricks-docs/structured-streaming-foreach/)** (captured notes) — `streamingDF.writeStream.foreachBatch(fn)` applies batch functions to each micro-batch; required for MERGE in streaming (see I5 #8). **At-least-once** by default; use `batchId` → `txnAppId`/`txnVersion` for exactly-once on Delta. Does **not** work with continuous mode (use `foreach()` instead). Stateful operators: must consume entire batch or query fails next batch. Empty DataFrame: Delta OPTIMIZE writes empty batches → must handle `isEmpty()`. DBR 14.0 (Standard mode): `print()` → driver logs; `dbutils.widgets` unavailable; objects must be serializable. Multiple sinks: prefer multiple SS writers (foreachBatch serializes writes → latency). Error handling: propagate to orchestrator (Jobs/Airflow); Delta writes use `txnAppId`/`txnVersion` — no local retry. Exception table: transient → catch+retry/DLQ; idempotent duplicates → suppress; logic/schema/critical → propagate. DLQ pattern: split valid/invalid per micro-batch → separate Delta tables, both with `txnVersion`/`txnAppId`. Was name-dropped in I5 merge note. I2 still ⬜. **batch** (full recompute each run; cost and latency scale with data size), **stateful** (streaming; tracks observed records over time; **watermarks are required** — omitting them causes state to accumulate infinitely, leading to slowdowns and OOM); **incremental** (materialized views auto-track source changes and update aggregates on refresh — the recommended alternative to repeated batch queries or stateful streaming for pre-computed aggregates); **approximate** (`approx_count_distinct`, `approx_percentile`, `approx_top_k`, `TABLESAMPLE` — trade precision for speed on very large datasets).
...
6. **Reference — [Connect to Lakebase (streaming sink)](sources/databricks-docs/streaming-lakebase/)** (captured notes) — SS sink for writing TO Lakebase (Public Preview); DBR 18+; classic compute only — **no serverless, no Lakeflow Declarative Pipelines**. Two connection methods: UC-registered (`.toTable("catalog.schema.table")`, auto-auth, creates table if missing) vs non-UC (`format("postgresql")` + `endpoint`/`dbtable`). Only Lakebase supported — not external PostgreSQL-compatible DBs. `batchsize` 1000 / `batchinterval` 100ms (flush on either); backpressure propagated upstream. Upsert via `INSERT INTO ... ON CONFLICT DO UPDATE` when `upsertkey` or PK found; plain insert otherwise; `upsertkey` must match PK exactly (mismatch = fail); sorts rows by key per batch (deadlock prevention); no complex types as key. Table/schema: simple identifiers only (no hyphens/quoted names). Connection pooling: 1 task/connection by default. Auto-retries transient JDBC errors. Triggers: all four supported; output modes: update+append ✓, complete ✗. Unrecognized options → `JDBC_STREAMING_SINK_INVALID_OPTIONS`. E8 still ⬜.
